...
export function AuthProvider({ children }) {
  ...
  const logout = useCallback(async () => { await api.logout().catch(() => {}); setState({ loading: false, user: null }); if (location.pathname.startsWith('/admin')) location.href = '/'; }, []);
  ...
  const value = useMemo(() => ({ ...state, authenticated: Boolean(state.user), isAdmin: state.user?.role === 'admin', loginOpen, setLoginOpen, login, register, logout, changePassword, updateProfile,refresh }), [state, loginOpen, login, register, logout, changePassword,updateProfile, refresh]);
