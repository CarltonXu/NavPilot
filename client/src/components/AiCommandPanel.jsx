...
export default function AiCommandPanel({scope='personal',initialText='',onExecuted,compact=false}){
  ...
  return <div className={`ai-command-panel ${compact?'compact':''}`}>
    ...
    {!plan?<button className="icon-btn primary assistant-primary" disabled={loading||!text.trim()} onClick={parse}>{t(loading?'ai.parsing':'ai.plan')}</button>:<div className="ai-plan-preview">
      ...
      <div className="ai-operation-list">{plan.operations.map((operation,index)=>{const depth=categoryDepth(operation);return <article className={`ai-operation ${operation.op==='category.create'?'category-create-operation':''} ${operation.destructive?'destructive':''}`} style={{'--operation-depth':depth}} key={index}><div className="ai-operation-heading"><span className="tag">{operationTitle(operation.op,locale)}</span>{operation.op==='category.create'&&<span className="ai-category-level">{locale==='en'?`Level ${depth+1}`:`${depth+1} 级分类`}</span>}{operation.destructive&&<strong>{locale==='en'?'Destructive':'破坏性操作'}</strong>}</div><PreviewValue operation={operation}/></article>;})}</div>
      {plan.status==='draft'&&<label className={`destructive-confirm plan-approval ${plan.destructive?'destructive':''}`}><input type="checkbox" checked={approved} onChange={e=>setApproved(e.target.checked)}/><span>{t(plan.destructive?'ai.approveDestructive':'ai.approvePlan')}</span></label>}
      <div className="modal-actions">{plan.status==='executed'?<><button className="icon-btn" disabled={undoing} onClick={undo}>{locale==='en'?'Undo':'撤销'}</button><button className="icon-btn primary" onClick={continueConversation}>{locale==='en'?'Continue':'继续对话'}</button></>:plan.status==='undone'?<><span className="tag">{locale==='en'?'Undone':'已撤销'}</span><button className="icon-btn primary" onClick={continueConversation}>{locale==='en'?'Continue':'继续对话'}</button></>:<button className="icon-btn primary" disabled={loading||!approved} onClick={execute}>{loading?t('ai.executing'):t('ai.confirmExecute')}</button>}</div>
    </div>}
  </div>;
}
