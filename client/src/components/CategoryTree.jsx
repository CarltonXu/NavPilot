...
function InlineRename({ category, onRename, onCancel }) {
  ...
  return <div className="category-inline-create category-inline-rename"><div className="category-inline-fields"><input autoFocus value={name} aria-label={t('category.renameNamed',{name:category.name})} onFocus={event=>event.target.select()} onChange={event=>setName(event.target.value)} onKeyDown={event=>{if(event.key==='Enter'){event.preventDefault();submit();}if(event.key==='Escape')onCancel();}}/><button type="button" disabled={saving||!name.trim()} aria-label={t('common.confirm')} onClick={submit}><Icon name="check" size={15}/></button><button type="button" aria-label={t('common.cancel')} onClick={onCancel}><Icon name="close" size={15}/></button></div>{error&&<div className="error-text">{error}</div>}</div>;
}

function Node({ node, depth, active, expanded, setExpanded, counts, manageable, creating, setCreating, editing, setEditing, onSelect, onCreate, onRename, onDelete, onDropItems }) {
  ...
  return <div className="category-tree-node" role="treeitem" aria-level={depth} aria-expanded={hasChildren?open:undefined} aria-selected={active===node.id}>
    <div className={`category-tree-row ${dragOver?'drop-target':''}`} style={{'--tree-depth':depth-1}} onDragOver={event=>{if(manageable&&onDropItems){event.preventDefault();event.stopPropagation();event.dataTransfer.dropEffect='move';setDragOver(true);}}} onDragLeave={event=>{if(!event.currentTarget.contains(event.relatedTarget))setDragOver(false);}} onDrop={event=>{if(!manageable||!onDropItems)return;event.preventDefault();event.stopPropagation();setDragOver(false);onDropItems(event,node.id);}}>
      ...
      <button className={`category-tree-select ${active===node.id?'active':''}`} aria-label={`${node.name} ${counts[node.id]||0}`} onClick={()=>onSelect(node.id)}><ContentIcon value={node.icon} size={16}/><span className="category-tree-name" title={node.name}>{node.name}</span><span className="count">{counts[node.id]||0}</span></button>
