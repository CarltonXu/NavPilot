...
test('AI infrastructure keeps durable jobs and conversations isolated by realm',async()=>{
  ...
  for(const table of['ai_jobs','resource_embeddings','ai_conversations','ai_messages','ai_reports','ai_runs','ai_run_events','notifications','ai_usage_events','ai_domain_policies'])assert.ok(db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(table),table);
  ...
  const conversations=createConversationService(db),conversation=conversations.create(user,current,'整理文档');conversations.append(conversation.id,user,{role:'user',content:'整理重复资源',metadata:{mode:'instruction'}});conversations.append(conversation.id,user,{role:'assistant',content:'已生成计划',metadata:{mode:'instruction',kind:'plan'}});assert.equal(conversations.messages(conversation.id,user).length,2);assert.equal(conversations.messages(conversation.id,{id:'other'}),null);assert.equal(conversations.list(user)[0].mode,'instruction');assert.equal(conversations.serialize(conversations.get(conversation.id,user)).mode,'instruction');
});
