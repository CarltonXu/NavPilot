...
const db = require('../db');
const { getGeoInfo } = require('./proxyGeoService');
...
function clientInfo(req) {
  ...
  return { ipPrefix: ipPrefix(req?.ip), country:geo.country, countrySource:geo.source, city:geo.city, citySource:geo.citySource, browser, os, device };
}
...
function analytics(req, eventName, { itemId = null, categoryId = null, scope = null, surface = null, viewMode = null, resource = null, properties = {}, eventId = null } = {}) {
  ...
  try { db.prepare(`INSERT INTO analytics_events(id,occurred_at_ms,event_name,user_id,item_id,category_id,scope,surface,view_mode,browser_family,os_family,device_class,ip_prefix,country_code,country_source,city_name,city_source,item_name,item_url,item_description,item_icon,item_owner_id,properties_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id,Date.now(),eventName,req?.auth?.user?.id||null,itemId,categoryId,scope,surface,viewMode,client.browser,client.os,client.device,client.ipPrefix,client.country,client.countrySource,client.city,client.citySource,resource?.name||null,resource?.url||null,resource?.description||null,resource?.icon||null,resource?.owner_id||null,JSON.stringify(safeMetadata(properties))); return true; } catch (error) { if (String(error.code).includes('CONSTRAINT')) return false; throw error; }
}
