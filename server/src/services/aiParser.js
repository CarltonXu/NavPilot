...
请只返回合法 JSON，不要输出多余文字或 Markdown。JSON 必须使用固定字段 items、name、url、description、category、icon、tags：
{"items":[{"name":"简洁站点名称","url":"包含 http:// 或 https:// 的完整链接","description":"不超过30字的一句话描述","category":"简短分类，如常用工具/研发/办公/文档/监控","icon":"单个 emoji","tags":["文档","AI"]}]}
...
Return valid JSON only, with no extra text or Markdown. Use exactly these keys: items, name, url, description, category, icon, tags:
{"items":[{"name":"concise site name","url":"full URL including http:// or https://","description":"one short sentence","category":"short category such as Tools/Engineering/Office/Docs/Monitoring","icon":"one emoji","tags":["docs","AI"]}]}
...
async function parseDescription(userText, { locale = "zh-CN" } = {}) {
  ...
  if (!apiKey) {
    throw aiError(
      "AI_NOT_CONFIGURED",
      "尚未配置 AI API Key，请在系统设置或服务端环境变量中配置",
      400,
    );
  }
  ...
  try {
    ...
    if (!Array.isArray(parsed.items))
      throw aiError("AI_INVALID_RESPONSE", "AI 返回格式不符合预期");
    return parsed.items;
  } catch (error) {
...
function persistItems(items, opts = {}) {
  ...
  if (scope === "personal" && !ownerId)
    throw aiError("AUTH_REQUIRED", "请先登录", 401);
  const findCategory = db.prepare(
    "SELECT id FROM categories WHERE scope=? AND owner_id IS ? AND name=? COLLATE NOCASE",
  );
  const insertCategory = db.prepare(
    "INSERT INTO categories(name,icon,scope,owner_id,sort_order) VALUES(?,?,?,?,(SELECT COALESCE(MAX(sort_order),-1)+1 FROM categories WHERE scope=? AND owner_id IS ?))",
  );
  const insertItem =
    db.prepare(`INSERT INTO items(name,url,icon,description,tags_json,category_id,sort_order,check_method,check_enabled,scope,owner_id)
  ...
  db.transaction(() => {
    for (const raw of items) {
      ...
      try {
        parsed = new URL(
          /^https?:\/\//i.test(raw.url) ? raw.url : `https://${raw.url}`,
        );
      } catch {
        throw aiError("AI_INVALID_RESPONSE", "AI 返回的链接无效", 400);
      }
      let categoryId = null;
      if (categoryName) {
  ...
  return db
    .prepare(
      `SELECT items.*,categories.name AS category_name,categories.icon AS category_icon FROM items LEFT JOIN categories ON categories.id=items.category_id WHERE items.id IN (${created.map(() => "?").join(",") || "NULL"})`,
    )
    .all(...created);
}
