...
function usage(db,{actorId=null,feature,model,started,success,errorCode=null,tokens={}}) {
  db.prepare('INSERT INTO ai_usage_events(id,actor_user_id,feature,provider_model,success,latency_ms,input_tokens,output_tokens,error_code,created_at_ms) VALUES(?,?,?,?,?,?,?,?,?,?)').run(crypto.randomUUID(),actorId,feature,model,success?1:0,Date.now()-started,tokens.prompt_tokens??null,tokens.total_tokens!=null?Math.max(0,tokens.total_tokens-(tokens.prompt_tokens||0)):null,errorCode,Date.now());
}
function createEmbeddingService(db = defaultDb) {
  ...
  async function embed(inputs,{actorId=null,feature='semantic_index'}={}) {
    ...
    try {
      ...
      usage(db,{actorId,feature,model:config.model,started,success:true,tokens:response.data?.usage||{}});
      return {vectors,model:config.model};
    } catch (error) {
      usage(db,{actorId,feature,model:config.model,started,success:false,errorCode:error.code||'AI_UPSTREAM_REQUEST_FAILED'});
  ...
  async function indexRealm(current,{actorId=null,progress=()=>{}}={}) {
    ...
    for (let offset=0;offset<pending.length;offset+=32) {
      const batch=pending.slice(offset,offset+32),result=await embed(batch.map((row)=>row.source),{actorId,feature:'semantic_index'});
  ...
  async function queryVector(query,actorId) {
    ...
    const {vectors}=await embed([query.slice(0,1000)],{actorId,feature:'semantic_search'}),vector=vectors[0];
  ...
  async function semanticScores(current,query,actorId) {
    const vector=await queryVector(query,actorId),rows=db.prepare("SELECT e.item_id,e.vector_json FROM resource_embeddings e JOIN items i ON i.id=e.item_id WHERE i.scope=? AND i.owner_id IS ?").all(current.scope,current.ownerId),scores=new Map();
