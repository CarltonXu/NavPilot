...
router.get("/", (req, res) => {
  ...
  const sql = `SELECT items.id,items.scope,items.name,items.url,items.description,items.icon,items.tags_json AS tagsJson,items.status,items.latency_ms AS latencyMs,
    ...
    WHERE (items.scope='public' OR (items.scope='personal' AND items.owner_id=?))
      AND (items.name LIKE ? ESCAPE '\\' OR items.url LIKE ? ESCAPE '\\' OR items.description LIKE ? ESCAPE '\\' OR items.tags_json LIKE ? ESCAPE '\\' OR categories.name LIKE ? ESCAPE '\\')
