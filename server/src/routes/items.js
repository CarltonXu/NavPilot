...
router.get('/',(req,res)=>{const scope=req.query.scope||'public';if(scope==='personal'&&!req.auth?.user)return res.status(401).json({code:'AUTH_REQUIRED',error:'请先登录'});try{const items=navigation.listItems(currentRealm(req,scope));if(!req.auth?.user)return res.json(items.map(item=>({...item,is_favorite:false,favorite_at_ms:null})));const favorites=new Map(db.prepare('SELECT item_id,created_at_ms FROM user_favorites WHERE user_id=?').all(req.auth.user.id).map(row=>[row.item_id,row.created_at_ms]));return res.json(items.map(item=>({...item,is_favorite:favorites.has(item.id),favorite_at_ms:favorites.get(item.id)||null})));}catch(error){return sendError(res,error,'ITEM_LIST_FAILED');}});
