...
const {availabilityForItems,availabilityDayForItem,clampDays}=require('../services/availabilityService');
...
function sendError(res,error,fallback){if(!error.status)console.error(`[items] ${fallback}`,error);return res.status(error.status||500).json({code:error.code||fallback,error:error.status?error.message:'操作失败'});}
...
router.post('/availability-summaries',(req,res)=>{
  ...
  try{
    ...
    const etag = '"' + crypto.createHash('sha1').update(JSON.stringify({mode,days,items:rows.map(r=>[r.id,r.status,r.latency_ms,r.last_checked_at,r.check_interval_minutes,r.next_check_at_ms])})).digest('hex') + '"';
...
function update(req,res){try{const current=currentRealm(req,req.item.scope);const result=db.transaction(()=>{const value=navigation.updateItem(current,req.item.id,req.body);if(current.scope==='public'&&req.body.access){access.replaceItemAccess(req.item.id,{...req.body.access,expectedVersion:undefined},req.auth.user,{incrementVersion:false});value.value=navigation.getItem(current,req.item.id);}auditWith(db,req,value.moved?'item.moved':'item.updated',{targetType:'item',targetId:req.item.id,metadata:{scope:current.scope,changedFields:value.changedFields,before:value.before,after:value.after,access:req.body.access||undefined}});return value;})();
  ...
  const monitoringChanged = ['url','checkMethod','checkTarget','checkEnabled','checkIntervalMinutes'].some((field)=>result.changedFields.includes(field));
  if(result.value.check_enabled && monitoringChanged) checkAndPersist(result.value,{triggerType:'configuration'}).catch(()=>{});
