...
- **全局导航助手**：
  ...
  - 支持 `⌘K` / `Ctrl+K` 快捷唤起，桌面为大型命令中心，移动端为全屏面板
...
- **状态探测**：
  ...
  - 支持定时自动探测（间隔可配置，默认 5 分钟）+ 后台"立即全部探测"按钮 + 单条目即时探测
...
AI 配置优先级为：**系统设置数据库覆盖 > 环境变量 > 内置默认值**。API Key 不会从接口完整返回，但后台保存时会以明文写入 `server/data/navpilot.db`、WAL 和备份；如果不希望密钥进入 SQLite，请仅使用 `.env` 配置并保护好文件权限。
...
- 全局助手面向所有访客，`⌘K`（macOS）或 `Ctrl+K`（Windows/Linux）可随时唤起。
- 匿名搜索仅返回公共资源；登录后搜索公共资源和自己的个人资源。搜索不到但输入看起来是网址时，界面会建议创建资源。
- 普通用户可在管理员授权后操作自己的个人空间；管理员可在助手内切换公共空间和个人空间。操作不要求先进入门户编辑态，但执行前必须检查并授权 AI 计划。
- 支持「丰富卡片」与「紧凑列表」两种展示，选择保存在浏览器 `navpilot_view_mode_v1`。
...
- 顶部「中文 / English」和两种展示模式对所有用户可用。

## 已知局限与生产加固

- UI 保存的 AI API Key 仍以明文存在 SQLite/WAL/备份中；敏感部署建议仅用环境变量。
